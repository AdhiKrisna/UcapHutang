...
struct HybridQwenExtractionService: DraftExtractionService {
    ...
    func extract(_ request: DraftExtractionRequest) async throws -> TransactionDraft {
        ...
        if let raw = rawResponse {
            do {
                captureOutput = try QwenOutputDecoder.decode(raw, for: request.flow, transcript: transcript)
            } catch {
                inferenceWarning = error.localizedDescription
        ...
        case .personal(let personal):
            let resolvedDate = TransactionDateResolver.resolve(
                transcript: transcript,
                modelComponents: personal.transactionTime,
                referenceNow: request.referenceDate
            )
        ...
        case .split(let split):
            let resolvedDate = TransactionDateResolver.resolve(
                transcript: transcript,
                modelComponents: split.transactionTime,
                referenceNow: request.referenceDate
            )
